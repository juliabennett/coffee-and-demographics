...
FindIntersectingZips <- function(city) { 
  #Returns a SpatialPolygonsDataFrame describing all ZIP codes that intersect 
  #  the given city. Uses stored intersection tests and ZIP code boundaries.
...
FindCoffee <- function(city, scope = "more") {
  #Compiles location information for coffee shops near the given city. 
  ...
  #  Uses stored intersection tests, city boundaries, and coffee data. 
  if (scope == "more"){
...
MapCities <- function(city, factualAtt = TRUE) {
  #Creates a map displaying boundaries for the given city. 
...
MapZipsAndPoints <- function(city) {
  #Creates a map displaying boundaries for the given city, boundaries for 
  #  every ZIP code intersecting this city, and locations for every coffee shop
  #  that is contained in one of these ZIP codes.
  dataCurr <- filter(data, cityName == city)
  cityShape <- cityShapes[cityShapes$NAMELSAD == dataCurr$placeName[1], ]
...
MapSquaresAndPoints <- function(city) { 
  #Creates a map displaying boundaries for the given city, boundaries for 
  #  both a "bad" and "good" grid approximately covering this city, and location 
  #  information for all coffee shops that are contained in a ZIP code 
  #  intersecting this city. 
  dataCurr <- filter(data, cityName == city)
  cityShape <- cityShapes[cityShapes$NAMELSAD == dataCurr$placeName[1], ]
  zipShapesCurr <- FindIntersectingZips(city)
  points <- FindCoffee(city, "more")
...
MakeIntroMap <- function(city) {
  #Creates a map displaying boundaries for the given city, boundaries for 
  #  a "good" grid approximately covering this city, and location information 
  #  for all coffee shops that are contained in either this city or this grid. 
  dataCurr <- filter(data, cityName == city)
  cityShape <- cityShapes[cityShapes$NAMELSAD == dataCurr$placeName[1], ]
  gridPoly <- squares[squares$placeName == dataCurr$placeName[1], ]
  gridPolyGood <- gridPoly[gridPoly$goodSquare == 1, ] 
...
MakePMFData <- function(city, comparison) {
  #Creates dataframe that can be used to plot the PMF of the coffee shop distribution
  #  associated to the given city. Also includes data for the PMF of the related 
  #  negative binomial distribution when comparison = TRUE. 
  dataCurr <- filter(data, cityName == city)
  pmf <- summarise(group_by(dataCurr, countInSquare), 
                   percent = 100*(n()/nrow(dataCurr)))
  pmf <- merge(pmf, data.frame(countInSquare = 0:15), all.y = TRUE) #creates a y-value for every x-value
  pmf[is.na(pmf)] <- 0
  pmf["source"] <- "observed"
  if (comparison) {
...
PlotDist <- function(city, comparison = FALSE) {
  #Plots the PMF of the coffee shop distribution associated to the given city. 
...
MakeStatsPlot <- function(city) { 
  #Plots variance vs. mean for all coffee shop distributions and includes a fitted
  #  quadratic curve. Highlights the point corresponding to the given city.
...
PlotPopDensByVars <- function(city, smoother) {
  ...
  #  fitted curves of the form indicated by smoother. Highlights the point 
